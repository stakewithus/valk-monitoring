<template>
  ...
  >
    ...
    <template v-slot:head(project)="data">
      <div class="sr-only">Project</div>
    </template>
    ...
    <template v-slot:cell(project)="data">
      <div>Default values for all projects</div>
    </template>
    ...
    <template v-slot:cell(actions)="data">
      <div class="d-flex align-items-center justify-content-center">
        <b-button
          ...
          variant="primary"
          size="sm"
          @click="editSetting(data.item.project,rawSettings)"
        >Edit</b-button>
        <b-button
          v-else
...
<style>
#defaultSettingsTable td:not(:last-child),
#defaultSettingsTable th:not(:last-child) {
  min-width: 150px;
}
</style>
<script>
export default {
  name: "default-settings-tables",
  props: {
    rawSettings: {
      type: Object
    },
